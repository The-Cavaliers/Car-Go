...
passport.deserializeUser((id, done) => models.Profile.where({ id }).fetch()
    .then((profile) => {
      if (!profile) {
        throw profile;
      }
...
const getOrCreateOAuthProfile = (type, oauthProfile, done) => models.Auth.where({ type, oauth_id: oauthProfile.id }).fetch({
  withRelated: ['profile'],
})
    .then((oauthAccount) => {
      if (oauthAccount) {
